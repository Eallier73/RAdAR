#!/usr/bin/env python3
"""
Reusable institutional Facebook extractor core for Radar.

This keeps the operational contract stable for the pipeline while replacing
the legacy Serper-based discovery path with direct Apify discovery for posts
and comments.
"""
...
from src.preprocessing.normalizar_semanas_canonicas import (
...
DEFAULT_OUTPUT_DIR = ROOT_DIR / "artifacts" / "runs" / "extraction" / "facebook"
...
WEEK_MODE_CANONICAL = "canonical_iso_week"
...
ABSORBED_COMMENTS_LOGIC = [
    "extract_post_text_from_item: multi-field recovery of parent post text from Apify items.",
    "normalize_candidate_post: keep the most informative post text and parent metadata available.",
]

DISCARDED_COMMENTS_LOGIC = [
    "Manual intermediate URL CSV as required input: removed to keep a direct Apify-only pipeline.",
    "Extra scraping against Facebook post URLs for parent enrichment: removed for cost and fragility.",
]

CANONICAL_MAIN_COLUMNS = [
    "tipo",
    "url",
    "post_url_padre",
    "fecha",
...
class FacebookExtractorError(Exception):
    """Base exception for the institutional extractor."""
...
def build_exact_range_name(start: date, end: date) -> str:
    label = f"semana_{format_spanish_date(start)}_{format_spanish_date(end)}_{end:%y}"
    return f"{start.isoformat()}_{label}"
...
def parse_date_str(value: str, field_name: str) -> date:
    try:
        ...
        raise ConfigurationError(f"{field_name} must use YYYY-MM-DD: {value}") from exc


def resolve_iso_week_start(raw_date: date) -> date:
    return normalize_to_iso_week_start(raw_date)


def resolve_iso_week_end(raw_date: date) -> date:
    return resolve_iso_week_start(raw_date) + timedelta(days=6)
...
def resolve_week_partition(since: date, until: date, mode: str) -> WeekPartition:
    if mode == WEEK_MODE_EXACT:
        naming_start = since
        naming_end = until
        week_name = build_exact_range_name(naming_start, naming_end)
    elif mode == WEEK_MODE_CANONICAL:
        naming_start = resolve_iso_week_start(since)
        naming_end = resolve_iso_week_end(until)
        week_name = build_week_folder_name(naming_start)
    else:  # pragma: no cover - argparse validates
        raise ConfigurationError(f"Unsupported weekly naming mode: {mode}")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Institutional Facebook extractor for Radar using direct Apify discovery and comment extraction.",
    )
    parser.add_argument("--since", default=None, help="Initial extraction date YYYY-MM-DD.")
    parser.add_argument("--until", default=None, help="Final inclusive extraction date YYYY-MM-DD.")
    parser.add_argument("--before", dest="before", default=None, help="Operational alias for --until.")
    parser.add_argument("--pages-file", default=None, help="CSV or JSON with target institutional pages.")
    parser.add_argument("--page-id", action="append", default=None, help="Filter one or more canonical pages by id.")
    parser.add_argument(
        "--pages",
        nargs="+",
        default=None,
        help="Legacy alias to filter pages by handle, label, or page id.",
    )
    parser.add_argument("--output-dir", default=None, help="Root output directory.")
    parser.add_argument("--run-id", default=None, help="Logical run identifier.")
    parser.add_argument("--log-level", default=None, help="Logging level: DEBUG, INFO, WARNING, ERROR.")
    parser.add_argument("--overwrite", action="store_true", default=None, help="Allow canonical overwrite.")
    parser.add_argument(
        "--publish-canonical",
        action="store_true",
        default=None,
        help="Publish canonical weekly copies at the artifact week root.",
    )
    parser.add_argument(
        "--week-name-mode",
        choices=WEEK_NAME_MODE_CHOICES,
        default=None,
        help="Weekly naming policy.",
    )
    parser.add_argument("--seed", type=int, default=None, help="Reproducible seed for post selection.")
    parser.add_argument(
        "--weekly-comment-cap-per-page",
        type=int,
        default=None,
        help="Weekly comment cap per page.",
    )
    parser.add_argument(
        "--max-posts-per-page-per-week",
        type=int,
        default=None,
        help="Maximum posts selected for comments per page and week.",
    )
    parser.add_argument("--max-comments-per-post", type=int, default=None, help="Maximum comments to retrieve per post.")
    parser.add_argument(
        "--discovery-results-per-page",
        type=int,
        default=None,
        help="Discovery depth for the posts actor.",
    )
    parser.add_argument(
        "--include-replies",
        action="store_true",
        default=None,
        help="Include replies when returned by the actor.",
    )
    parser.add_argument("--dry-run", action="store_true", default=None, help="Do not call Apify; only materialize metadata.")
    parser.add_argument("--apify-token-env", default=None, help="Name of the environment variable with the Apify token.")
    parser.add_argument("--actor-name-posts", default=None, help="Apify actor name for post discovery.")
    parser.add_argument("--actor-name-comments", default=None, help="Apify actor name for comment extraction.")
    parser.add_argument("--memory-mb", type=int, default=None, help="Requested memory for actor runs.")
    parser.add_argument("--timeout-seconds", type=int, default=None, help="Timeout for actor runs.")
    parser.add_argument("--config-file", default=None, help="Optional JSON file with base configuration.")
    parser.add_argument(
        "--no-prompt",
        action="store_true",
        default=None,
        help="Legacy no-op flag preserved for pipeline compatibility.",
    )
...
def normalize_page_id(raw_value: str) -> str:
    return "".join(ch for ch in str(raw_value).strip() if ch.isdigit())
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        if not required.issubset(reader.fieldnames or []):
            raise ConfigurationError(
                f"Pages file requires at least columns {sorted(required)}: {path}"
            )
        for row in reader:
            ...
            page_url = str(row.get("page_url", "")).strip() or default_page_url(page_id, page_handle)
            if not page_handle:
...
def load_pages_from_json(path: Path) -> list[PageTarget]:
    ...
    if not isinstance(payload, list):
        raise ConfigurationError("--pages-file JSON must contain a list.")
    ...
    for item in payload:
        ...
        page_url = str(item.get("page_url", "")).strip() or default_page_url(page_id, page_handle)
        if not page_handle:
            page_handle = extract_handle_from_facebook_url(page_url)
        pages.append(
            PageTarget(
...
def load_pages_from_file(path: Path) -> list[PageTarget]:
    ...
    raise ConfigurationError(f"Unsupported pages-file format: {path}")
...
def resolve_pages(merged: dict[str, Any]) -> tuple[list[PageTarget], str]:
    ...
    if pages_file_raw:
        ...
        if not pages_file.exists():
            raise ConfigurationError(f"--pages-file does not exist: {pages_file}")
    ...
    selected_ids = {
        ...
        for value in (merged.get("page_id") or [])
        if normalize_page_id(value)
    }
    legacy_filters = {normalize_target_handle(value) for value in (merged.get("pages") or []) if value}
    if selected_ids:
        ...
        if missing:
            raise ConfigurationError(
                "Requested --page-id outside the active canonical institutional set: "
                + ", ".join(sorted(missing))
            )
    if legacy_filters:
        active_pages = [
            ...
            if page.page_id in legacy_filters
            or normalize_target_handle(page.page_handle) in legacy_filters
            or normalize_target_handle(page.page_label) in legacy_filters
        ]
        if not active_pages:
            raise ConfigurationError("Legacy page filter (--pages) did not match any active canonical page.")

    if not active_pages:
        raise ConfigurationError("Final institutional page selection is empty.")

    invalid = [page.page_id for page in active_pages if page.page_id not in CANONICAL_PAGES]
    if invalid:
        raise ConfigurationError(
            "This component is restricted to institutional Facebook pages from Tampico. "
            "Pages outside the canonical set must remain disabled: "
            + ", ".join(sorted(invalid))
        )
...
def resolve_config(
    ...
    *,
    ...
    if not since_raw or not until_raw:
        raise ConfigurationError("You must define --since and --until/--before, or provide them in --config-file.")

    since = parse_date_str(str(since_raw), "since")
    until = parse_date_str(str(until_raw), "until")
    if until < since:
        raise ConfigurationError("until cannot be earlier than since.")
    ...
    if discovery_results_per_page > 100:
        raise ConfigurationError("discovery_results_per_page cannot exceed 100.")
    ...
    return ResolvedConfig(
        ...
        max_comments_per_post=max_comments,
        discovery_results_per_page=discovery_results_per_page,
        include_replies=bool(merged.get("include_replies")) if merged.get("include_replies") is not None else True,
...
def ensure_publish_target(target_path: Path, overwrite: bool) -> None:
    if target_path.exists() and not overwrite:
        raise OutputWriteError(f"Canonical artifact already exists and requires --overwrite: {target_path}")
...
def call_actor(
    ...
    if client is None:
        raise ApiExecutionError("Apify client is not available.")
    ...
    if not run:
        raise ApiExecutionError(f"Actor {actor_name} returned no result.")

    dataset_id = run.get("defaultDatasetId")
    if not dataset_id:
        raise ApiExecutionError(f"Actor {actor_name} did not return a dataset.")
...
def build_apify_client(config: ResolvedConfig) -> Any:
    if APIFY_IMPORT_ERROR is not None:
        raise PlatformDependencyError(
            "Missing critical dependency: apify-client. Install it in the ops environment."
        )
    token = os.environ.get(config.apify_token_env, "").strip()
    if not token:
        raise ApiAuthenticationError(
            f"Missing token in environment variable {config.apify_token_env}."
        )
...
def discover_candidate_posts(
    ...
    logger.info(
        "Discovering candidate posts | page_id=%s page_label=%s results_limit=%s since=%s until=%s",
        page.page_id,
        page.page_label,
    ...
    logger.info(
        "Posts actor completed | page_id=%s status=%s usage_usd=%.4f items=%s",
        page.page_id,
        run_meta["status"],
    ...
    for item in items:
        ...
        if not date_in_range(post.created_dt, config.since, config.until):
            continue
        existing = deduped.get(post.post_url)
        if existing is None or len(post.post_text) > len(existing.post_text):
            deduped[post.post_url] = post
...
def select_posts_for_comment_sampling(
    ...
    logger.info(
        "Posts selected by sampling | selected=%s candidates=%s rule=%s",
        len(selected),
        len(candidate_posts),
        POST_SELECTION_RULE,
    )
...
def comment_in_historical_range(value: datetime | None, since: date, until: date) -> bool:
    if value is None:
        return False
    current = value.date()
    return since <= current <= until
...
def extract_comments_for_post(
    ...
    *,
    ...
    run_input = {
        ...
        "onlyCommentsNewerThan": config.since.isoformat(),
    }

    logger.info(
        "Extracting comments | page_id=%s post_id=%s requested=%s remaining_page_cap=%s include_replies=%s",
        page.page_id,
        post.post_id,
        requested,
        remaining_page_cap,
    ...
    for item in items:
        ...
        if not comment_in_historical_range(created_dt, config.since, config.until):
            continue
    ...
    logger.info(
        "Comments materialized | page_id=%s post_id=%s status=%s usage_usd=%.4f rows=%s enriched=%s",
        page.page_id,
        post.post_id,
    ...
    return rows, [], requested, float(run_meta["usageTotalUsd"]), enriched
...
def build_selection_audit(
    ...
    for candidate in page_result.candidate_posts:
        ...
        if selected_post is not None:
            ...
            decision_reason = "Selected by temporal coverage rule and processed under the weekly cap."
            ...
            decision_reason = "Excluded by temporal even-spacing sampling before comment extraction."
...
def extract_page_batch(
    ...
    for post in processing_queue:
        if page_comments_saved >= config.weekly_comment_cap_per_page:
            ...
            logger.info(
                "Weekly cap reached | page_id=%s page_label=%s cap=%s comments_saved=%s",
                page.page_id,
                page.page_label,
    ...
    for page_result in page_results:
        ...
        for comment in page_result.comments:
            rows.append(
                {
                    "tipo": "COMENTARIO",
                    "url": comment.comment_url or comment.post_url,
                    "post_url_padre": comment.post_url,
                    "fecha": normalize_utc_timestamp(comment.created_time),
                    "texto": comment.text,
                    "num_comentarios": "",
                }
            )
    if not rows:
        return pd.DataFrame(columns=CANONICAL_MAIN_COLUMNS)
    return pd.DataFrame(rows, columns=CANONICAL_MAIN_COLUMNS)


def build_detailed_main_dataframe(page_results: Sequence[PageExecutionResult], config: ResolvedConfig) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    for page_result in page_results:
        for post in page_result.selected_posts:
            rows.append(
    ...
    if not rows:
        return pd.DataFrame(columns=DETAILED_MAIN_COLUMNS)
    return pd.DataFrame(rows, columns=DETAILED_MAIN_COLUMNS)


def build_posts_dataframe(page_results: Sequence[PageExecutionResult], config: ResolvedConfig) -> pd.DataFrame:
    main_df = build_detailed_main_dataframe(page_results, config)
    if main_df.empty:
        return pd.DataFrame(columns=DETAILED_MAIN_COLUMNS)
...
def build_notes(config: ResolvedConfig, *, status: str, dry_run: bool) -> list[str]:
    notes = [
        "Extractor restricted to institutional Facebook pages from Tampico.",
        f"Sampling mode: {SAMPLING_MODE}.",
        f"Post selection rule: {POST_SELECTION_RULE}.",
        f"Comment processing rule: {COMMENT_PROCESSING_RULE}.",
        "Primary analytical unit is the comment; the parent post is kept for context and traceability.",
        "Direct Apify discovery replaces the legacy Serper URL acquisition step.",
    ]
    if config.include_replies:
        notes.append("Replies included to preserve comparability with the historical comment extractor.")
    else:
        notes.append("Replies disabled by configuration.")
    if dry_run:
        notes.append("Dry-run active: no Apify actors were executed.")
    if status != "success":
        notes.append("The run completed with recoverable incidents or partial failures.")
    return notes
...
def publish_canonical_artifacts(
    ...
    for key, source_path in artifact_paths.items():
        ...
        logger.info("Published canonical artifact | source=%s target=%s", source_path, target_path)
    return published
...
def persist_outputs(
    ...
    *,
    ...
    add_artifact_record(artifact_records, main_path, "dataset", "Institutional Facebook extraction with parent posts and comments.")

    posts_path = config.run_dir / POSTS_FILENAME
    dataframe_to_csv(posts_path, posts_df)
    artifact_paths["posts_csv"] = posts_path
    add_artifact_record(artifact_records, posts_path, "dataset", "Posts selected for comment extraction under the sampling policy.")
    ...
    add_artifact_record(artifact_records, comments_path, "dataset", "Comments materialized for the institutional corpus.")

    audit_path = config.run_dir / AUDIT_FILENAME
    dataframe_to_csv(audit_path, audit_df)
    artifact_paths["audit_csv"] = audit_path
    add_artifact_record(artifact_records, audit_path, "audit", "Post selection and sampling audit trail.")
    ...
    add_artifact_record(artifact_records, summary_path, "summary", "Operational and cost summary.")
    ...
    add_artifact_record(artifact_records, params_path, "parameters", "Effective parameters of the run.")
    ...
    add_artifact_record(artifact_records, metadata_path, "metadata", "Full metadata and traceability for the run.")
    ...
    if errors:
        ...
        add_artifact_record(artifact_records, errors_path, "errors", "Recoverable errors captured during the run.")

    log_path = config.run_dir / LOG_FILENAME
    if log_path.exists():
        artifact_paths["run_log"] = log_path
        add_artifact_record(artifact_records, log_path, "log", "Operational log for the run.")
    ...
    add_artifact_record(artifact_records, manifest_path, "manifest", "Artifact inventory for the run.")
    manifest_payload = [serialize_for_json(record) for record in artifact_records]
    write_json_file(manifest_path, manifest_payload)
...
def write_failure_artifacts(
    ...
    *,
    ...
    add_artifact_record(records, summary_path, "summary", "Fatal failure summary.")
    add_artifact_record(records, metadata_path, "metadata", "Fatal failure metadata.")
    add_artifact_record(records, params_path, "parameters", "Effective parameters for the failed run.")
    add_artifact_record(records, errors_path, "errors", "Fatal errors captured for the failed run.")
    write_json_file(manifest_path, [serialize_for_json(record) for record in records])
    logger.error("Failure artifacts written | run_dir=%s", config.run_dir)
...
def run_extraction(config: ResolvedConfig, logger: logging.Logger) -> RunExecutionResult:
    ...
    logger.info(
        "Run start | run_id=%s week_name=%s output_root=%s week_name_mode=%s",
        config.run_id,
    ...
    logger.info(
        "Effective parameters | since=%s until=%s pages=%s seed=%s weekly_cap=%s max_posts=%s max_comments_per_post=%s include_replies=%s dry_run=%s",
        config.since,
        config.until,
        len(config.pages),
        config.seed,
    ...
    logger.info(
        "Target institutional pages | pages=%s",
        ", ".join(f"{page.page_label}:{page.page_id}:{page.page_handle or 'no_handle'}" for page in config.pages),
    )

    if config.dry_run:
        logger.info("Dry-run active; no Apify actor will be executed.")
    ...
    for index, page in enumerate(config.pages, start=1):
        logger.info(
            "Processing page %s/%s | page_id=%s page_label=%s page_handle=%s",
            index,
            len(config.pages),
            page.page_id,
            page.page_label,
    ...
    logger.info(
        "Final summary | status=%s candidate_posts=%s posts_selected=%s comments_saved=%s total_usage_usd=%.4f",
        status,
...
def main(
    ...
    *,
    ...
    try:
        ...
        logger.error("Fatal Apify authentication error: %s", exc)
        try:
            ...
            logger.error("Unable to write failure artifacts: %s", write_exc)
        return int(ExitCode.FAILED_API)
    except PlatformDependencyError as exc:
        logger.error("Fatal dependency/platform error: %s", exc)
        try:
            write_failure_artifacts(config, logger, error=exc, exit_code=ExitCode.FAILED_API)
        except Exception as write_exc:  # pragma: no cover - defensive
            logger.error("Unable to write failure artifacts: %s", write_exc)
        ...
        logger.error("Fatal Apify/platform error: %s", exc)
        try:
            write_failure_artifacts(config, logger, error=exc, exit_code=ExitCode.FAILED_API)
        except Exception as write_exc:  # pragma: no cover - defensive
            logger.error("Unable to write failure artifacts: %s", write_exc)
        ...
        logger.error("Unhandled fatal error: %s", exc)
        try:
            write_failure_artifacts(config, logger, error=exc, exit_code=ExitCode.FAILED_API)
