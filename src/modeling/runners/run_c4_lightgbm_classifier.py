...
from ..core.config import CLASS_LABELS_5


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="C4 | Familia LightGBMClassifier multiclase bajo walk-forward temporal.",
    )
    add_common_classification_args(parser, default_run_id="C4_v1_clean")
    parser.set_defaults(
        feature_mode="all",
...
def main() -> None:
    ...
    result = run_classification_experiment(
        ...
        experiment_id="C4",
        family="clasificacion_lightgbm",
        model_name="lightgbm_classifier",
        estimator=build_estimator(args),
        model_params=build_model_params(args),
        comentarios=(
            "LightGBMClassifier limpio con walk-forward temporal | "
            f"hipotesis={args.hypothesis_note or 'lightgbm_clasificacion_baseline'} | "
            "sin tuning interno"
        ),
