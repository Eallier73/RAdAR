...
def has_twitter_file(week_dir: Path) -> bool:
    for f in week_dir.iterdir():
        if f.is_file() and f.name.lower().endswith("_twitter.csv"):
            return True
    return False


def output_path_for_week(week_dir: Path) -> Path:
    return week_dir / f"{week_dir.name}_twitter.csv"
...
def main() -> int:
    ...
    p.add_argument(
        "--src",
        required=True,
        help="Directorio externo con Tweets_MV_GT_*.csv.",
    )
    p.add_argument(
        "--dest",
    ...
    log_path = PREPROCESSING_LOGS_DIR / f"fill_twitter_weekly_from_monthly_{ts}.csv"
