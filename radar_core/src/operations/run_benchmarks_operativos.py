...
from ..shared.common_runtime_logging import log_event


ROOT_DIR = Path(__file__).resolve().parents[2]

REGISTRY_PATH = ROOT_DIR / "experiments" / "audit" / "registro_operacion_controlada_radar.json"
GRID_WORKBOOK_PATH = ROOT_DIR / "experiments" / "audit" / "grid_experimentos_radar.xlsx"
MASTER_WORKBOOK_PATH = ROOT_DIR / "experiments" / "audit" / "tabla_maestra_experimentos_radar.xlsx"
INVENTORY_PATH = ROOT_DIR / "experiments" / "audit" / "inventario_experimentos_radar.json"
DEFAULT_REPORT_JSON_PATH = (
    ROOT_DIR / "experiments" / "audit" / "auditoria_benchmarks_operativos_controlados.json"
)
DEFAULT_REPORT_MD_PATH = (
    ROOT_DIR / "experiments" / "audit" / "auditoria_benchmarks_operativos_controlados.md"
)
DEFAULT_PYTHON_BIN = Path(os.environ.get("RADAR_MODELING_PYTHON", sys.executable))
...
FROZEN_EXECUTION_SPECS = {
    "benchmark_numerico_puro_vigente": {
        "runner": ROOT_DIR / "src" / "modeling" / "runners" / "run_e1_ridge_clean.py",
        "args": [
            "--reference-run-id",
            "E1_v5_clean",
            "--target-mode",
    ...
    "benchmark_operativo_riesgo_vigente": {
        "runner": ROOT_DIR / "src" / "modeling" / "runners" / "run_e9_stacking.py",
        "args": [
            "--reference-run-id",
            "E1_v5_clean",
            ...
            "--table-path",
            str(ROOT_DIR / "experiments" / "audit" / "tabla_maestra_experimentos_radar_e9_curada.xlsx"),
            "--horizons",
            "1,2,3,4",
            "--initial-train-size",
            "12",
