...
from ..core.config import DATE_COLUMN, ROOT_DIR
from ..core.evaluation import compute_loss_h, compute_radar_metrics, compute_total_radar_loss
from ..tracking.experiment_logger import DEFAULT_RUNS_DIR, DEFAULT_WORKBOOK, RadarExperimentTracker
    ...
    for summary in horizon_summaries:
        lines.extend(
            [
                ...
                f"- accuracy_selector: `{summary['selector_accuracy']:.6f}`",
                f"- balanced_accuracy_selector: `{summary['selector_balanced_accuracy']}`",
                f"- loss_h_E10: `{summary['system_loss_h']:.6f}`",
                f"- benchmark_fijo_loss_h: `{summary['benchmark_fixed_loss_h']:.6f}`",
                "",
            ]
        )
    lines.extend(
...
def run_horizon(
    *,
    ...
    rows_summary = {
        ...
        "system_loss_h": float(system_metrics["loss_h"]),
