...
from ..core.data_master import load_master_dataset
from ..core.evaluation import compute_loss_h, compute_radar_metrics, compute_total_radar_loss
from ..tracking.experiment_logger import DEFAULT_WORKBOOK, RadarExperimentTracker
    ...
    phase1_sources.write_text(
        "\n".join(
            [
                ...
                f"- `experiments/audit/tabla_maestra_experimentos_radar.xlsx`",
                f"- `src/modeling/reporting/build_experiments_master_table.py`",
            ]
        ),
        encoding="utf-8",
    )
