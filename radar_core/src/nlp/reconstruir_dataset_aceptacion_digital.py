...
Pipeline reproducido:
    datos_ML_0.xlsx
        -> datos_ML_3.xlsx              (poda manual 1)
        -> datos_ML_4.xlsx              (lags via Lageado_Datos_ML)
        -> datos_ML_5.xlsx              (poda manual 2)
        -> datos_ML_master.xlsx         (rama legacy)
        -> datos_ML_master_1.xlsx       (snapshot legacy)
        -> datos_ML_master_2.xlsx       (rename + correccion de leads)
        -> datos_ML_master_3.xlsx       (poda + rename)
        -> datos_ML_master_4.xlsx       (cambio de target a aceptacion digital)
        -> datos_ML_master_indice_aceptación_digital.xlsx

Por defecto escribe todo en un directorio separado para no sobreescribir los
...
from Lageado_Datos_ML import build_readme, generate_lags, output_sheet_name, write_output
...
MASTER_CHAIN_FEATURE_COLUMNS = ["sentimiento_medios", *V5_NETO_COLUMNS]
...
def build_master_1(ml0_train: pd.DataFrame) -> pd.DataFrame:
    ...
    for horizon in (1, 2, 3, 4):
        base[f"lead_tarjet_{horizon}w_serie_4e"] = base["serie_4e"].shift(horizon - 1)
    ordered_columns = [
        "fecha_inicio_semana",
        "semana_iso",
        "iso_year",
...
def main() -> None:
    ...
    write_single_sheet(
        output_dir / "datos_ML_master_indice_aceptación_digital.xlsx",
        "Sheet1",
        master_indice,
    )
    print(f"- datos_ML_master_indice_aceptación_digital.xlsx: {len(master_indice.columns)} columnas")
    print("=" * 72)
