...
def summarize_catboost_trace(catboost_trace: list[dict[str, object]]) -> dict[str, object]:
    best_iterations = [float(item["best_params"]["iterations"]) for item in catboost_trace]
    best_depths = [float(item["best_params"]["depth"]) for item in catboost_trace]
    best_learning_rates = [float(item["best_params"]["learning_rate"]) for item in catboost_trace]
    best_l2 = [float(item["best_params"]["l2_leaf_reg"]) for item in catboost_trace]
    best_subsamples = [float(item["best_params"]["subsample"]) for item in catboost_trace]
    return {
...
def main() -> None:
    ...
    for horizon in args.horizons:
        ...
        if args.use_inner_tuning:
            ...
            tuning_summary = summarize_catboost_trace(catboost_trace)
