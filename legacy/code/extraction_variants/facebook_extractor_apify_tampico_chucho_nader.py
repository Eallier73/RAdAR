...
def obtener_comentarios_batch(
    ...
    apply_comment_since_filter: bool = True,
) -> list:
    """Corre facebook-comments-scraper para un batch de URLs."""
    start_urls = [{"url": url} for url in post_urls]
    view_option = VIEW_OPTION_MAP.get(comments_mode, VIEW_OPTION_MAP["all"])
    # En el actor actual, `resultsLimit` es un límite total por corrida.
    # Para aproximar "máx por post" en un batch, escalamos por número de URLs.
    results_limit = max_comments * max(1, len(post_urls))
    ...
    if since and apply_comment_since_filter:
...
def procesar_items_comentarios(items: list, dedup_comments: bool = True) -> List[dict]:
...
def fase2_comentarios(
    ...
    dedup_comments: bool,
    comments_mode: str,
    apply_comment_since_filter: bool,
):
    ...
    print(f"  🧹 Deduplicación texto: {'sí' if dedup_comments else 'no'}")
    print(f"  🔎 Modo comentarios: {comments_mode}")
    print(f"  ⏱️  Filtro fecha comentario: {'sí' if apply_comment_since_filter else 'no'}")
    ...
    for i in range(0, len(urls), batch_size):
        ...
        items = obtener_comentarios_batch(
            ...
            apply_comment_since_filter=apply_comment_since_filter,
        )
        filas = procesar_items_comentarios(items, dedup_comments=dedup_comments)
...
Ejemplos:
  ...
  python apify_comentarios.py --solo-comentarios \\
    ...
    parser.add_argument("--comments-mode", choices=["all", "newest", "most-relevant"],
                        default="all",
                        help="Orden de comentarios en Facebook (default: all)")
    parser.add_argument("--no-comment-since-filter", action="store_true",
                        help="No aplicar onlyCommentsNewerThan aunque exista --since")
    parser.add_argument("--dedup-comments", dest="dedup_comments", action="store_true",
                        help="Deduplicar comentarios repetidos por post (default)")
    parser.add_argument("--no-dedup-comments", dest="dedup_comments", action="store_false",
                        help="No deduplicar comentarios repetidos por post")
    parser.set_defaults(dedup_comments=True)
...
def main():
    ...
    if run_fase2:
        ...
        fase2_comentarios(
            ...
            dedup_comments=args.dedup_comments,
            comments_mode=args.comments_mode,
            apply_comment_since_filter=not args.no_comment_since_filter,
        )
